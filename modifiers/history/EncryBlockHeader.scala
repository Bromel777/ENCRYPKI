...
object EncryBlockHeaderSerializer extends Serializer[EncryBlockHeader] {
  ...
  override def parseBytes(bytes: Array[Byte]): Try[EncryBlockHeader] = Try{
    ...
    val parrentId = ModifierId @@ bytes.slice(1,33)
    val txMerkleRoot = Digest32 @@ bytes.slice(33,65)
    val timestamp = Longs.fromByteArray(bytes.slice(65,73))
    val height = Ints.fromByteArray(bytes.slice(73,77))
    val nonce = Longs.fromByteArray(bytes.slice(77,81))
    val targetDiff = Ints.fromByteArray(bytes.slice(81,85))
    EncryBlockHeader(version,parrentId,txMerkleRoot,timestamp,height,nonce,targetDiff)
  }
}
